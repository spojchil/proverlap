 ...
 *
 * <p>封装 GitHub App 认证流程：
 *
 * <ol>
 *   <li>私钥 + App ID 生成 JWT（RS256，10 分钟有效期）
 *   <li>JWT 换取安装访问令牌（60 分钟有效期，本地缓存 50 分钟）
 *   <li>使用安装令牌调用 GitHub REST API
 * </ol>
 *
...
public class GitHubClient {
     ...
     *
     * <ol>
     *   <li>GitHub App Installation Token（需配置 installationId，Webhook 模式）
     *   <li>Personal Access Token（需配置 token，API 模式最低配置）
     * </ol>
     *
        ...
        if (props.getToken() != null && !props.getToken().isBlank()) {
            try {
                ...
                throw new IllegalStateException(
                        "PAT Token 认证失败，请检查 GITHUB_TOKEN 是否正确。错误: " + e.getMessage(), e);
            }
        }

        // 两种方式都未配置 → 给出清晰的配置指引
        String msg =
                "未配置 GitHub 认证。请配置以下任一方式：\n"
    ...
    private String fetchRepoFile(String owner, String repo, String path, String token, String ref) {
        try {
            ...
            return new String(
                    Base64.getDecoder().decode(objectMapper.readTree(raw).path("content").asText()),
    ...
    public void updateCheckRun(
        ...
        Map<String, Object> body =
                Map.of(
                        ...
                        java.time.Instant.now().toString(),
                        "output",
                        Map.of("title", title, "summary", summary));
        restClient
                .patch()
    ...
    public void postReview(
            String owner, String repo, int prNumber, String body, long installationId) {
        String token = obtainToken(installationId);
     ...
     * 获取安装访问令牌（优先从缓存读取）。
     *
     * <p>tokenCache 为 ConcurrentHashMap，快速路径无锁读支持多 PR 真正并发。 DCL + synchronized：get+put 非原子，同一
     * installationId 只有一个线程刷新令牌。
     */
    private String obtainToken(long installationId) {
        ...
        synchronized (tokenCache) {
            ...
            tokenCache.put(
                    installationId, new CachedToken(token, Instant.now().plus(TOKEN_CACHE_TTL)));
            return token;
        }
    }
     ...
     *
     * <p>使用 Java 标准库 {@link Signature} 完成签名，不依赖外部 JWT 库。
     */
     ...
     * 解析 PEM 格式 RSA 私钥。
     *
     * <p>根据 PEM 头区分格式：PKCS#1（BEGIN RSA PRIVATE KEY）走自建 DER 解析， PKCS#8（BEGIN PRIVATE KEY）走标准
     * PKCS8EncodedKeySpec。
     */
    ...
    /**
     * 解析 PKCS#1 DER 序列: SEQUENCE { version, modulus, publicExp, privateExp, prime1, prime2, exp1,
     * exp2, coeff }
     */
