 ...
 *
 * <p>创建两个 ChatModel Bean（modelA / modelB），分别对应模型插槽 A 和 B。 使用 OpenAI
 * 兼容接口，所有兼容该格式的模型均可接入（DeepSeek、智谱、Claude via API Gateway 等）。
 */
@Slf4j
@Configuration
...
public class LLMConfig {
    ...
    public void init() {
        log.info(
                "LLM 模型配置初始化: 模型 A = {} ({})",
                modelProperties.getModelA().getModelName(),
                modelProperties.getModelA().getBaseUrl());
        log.info(
                "LLM 模型配置初始化: 模型 B = {} ({})",
                modelProperties.getModelB().getModelName(),
                modelProperties.getModelB().getBaseUrl());
    }

    /**
     * 模型 A — 主审查模型。
     *
     * <p>承担代码逻辑、安全、架构维度审查。温度 0.3 保证输出确定性。
     */
    @Bean("modelA")
     ...
     * 模型 B — 交叉验证模型。
     *
     * <p>承担安全、逻辑维度的交叉验证。温度 0.3 保证与模型 A 可比对。
     */
    @Bean("modelB")
