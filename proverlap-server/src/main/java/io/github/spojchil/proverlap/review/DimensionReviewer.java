 ...
 * 维度 × 模型调度器。
 *
 * <p>解析 PR 标题获取类型（feat/fix/perf/...），查矩阵激活对应维度， 按维度独立审查，双模型 CV 用于安全+正确性，单模型用于次要维度。
 */
@Slf4j
@Service
public class DimensionReviewer {
    ...
    private static final Pattern CONVENTIONAL_TYPE =
            Pattern.compile(
    ...
    private static final Pattern ISSUE_FIX =
            Pattern.compile("^(fix(?:es)?|resolve(?:s)?)[:\\s]+#\\d+", Pattern.CASE_INSENSITIVE);

    /** 祈使动词 → PR 类型映射 */
    private static final java.util.Map<String, String> IMPERATIVE_TYPE =
            java.util.Map.ofEntries(
                    ...
                    java.util.Map.entry("refactor", "refactor"),
                            java.util.Map.entry("revert", "revert"),
                    ...
                    java.util.Map.entry("updated", "feat"),
                            java.util.Map.entry("deleted", "chore"));

    /** 维度 × 模型矩阵：PR 类型 → 维度任务列表 */
    private final Map<String, List<DimensionTask>> matrix;

    public DimensionReviewer(
    ...
    /** 单模型审查 */
    private CompletableFuture<DimensionResult> singleModelReview(
            DimensionTask task, String context) {
        return CompletableFuture.supplyAsync(
                () -> {
                    try {
                        ...
                        String formatted =
                                formatSingleModelFindings(task.dimension(), findings, raw);
                        return DimensionResult.of(task.dimension(), formatted, false, findings);
                    } catch (Exception e) {
                        log.error("单模型审查失败({}): {}", task.dimension(), e.getMessage());
                        return DimensionResult.of(
                                task.dimension(), "审查失败: " + e.getMessage(), false);
                    }
                },
                executor);
    }
    ...
    private CompletableFuture<DimensionResult> dualModelReview(DimensionTask task, String context) {
        CompletableFuture<List<Finding>> fa =
                CompletableFuture.supplyAsync(
                        () -> callAndParse(modelA, task.prompt(), context, "modelA"), executor);
        CompletableFuture<List<Finding>> fb =
                CompletableFuture.supplyAsync(
                        () -> callAndParse(modelB, task.prompt(), context, "modelB"), executor);

        return fa.thenCombine(
                        fb,
                        (findingsA, findingsB) -> {
                            CrossValidationResult cross =
                                    crossValidator.compare(findingsA, findingsB);
                            ...
                            return DimensionResult.of(
                                    task.dimension(), formatted, true, allFindings);
                        })
                .exceptionally(
                        e -> {
    ...
    /** 调用模型 + 解析 JSON */
    private List<Finding> callAndParse(
            ChatModel model, ReviewPrompt prompt, String context, String modelName) {
        try {
    ...
    private String callModel(ChatModel model, ReviewPrompt prompt, String context) {
        ChatResponse response =
                model.chat(List.of(SystemMessage.from(prompt.system()), UserMessage.from(context)));
    ...
    private List<DimensionTask> selectTasks(String prType, TierLevel tier) {
        ...
        if (tier == TierLevel.TIER_1) {
            // T1: 只保留第一个单模型维度
            return tasks.stream().filter(t -> t.modelCount() == 1).limit(1).toList();
        }

        if (tier == TierLevel.TIER_2) {
            // T2: 跳过设计维度（最高层，T3 才有）
            return tasks.stream().filter(t -> !"design".equals(t.dimension())).toList();
        }
    ...
    private static String formatSingleModelFindings(
            String dimension, List<Finding> findings, String raw) {
    ...
    private Map<String, List<DimensionTask>> buildMatrix() {
        return Map.of(
                ...
                "style", List.of(),
                "chore", List.of(),
                "test", List.of(DimensionTask.of("test", testCoveragePrompt, 1)),
                "build", List.of(),
                "ci", List.of());
    }
    ...
    public record DimensionResult(
            String dimension, String findingsText, boolean crossValidated, List<Finding> findings) {
        public static DimensionResult of(
                String dimension, String findingsText, boolean crossValidated) {
