...
RUN sed -i 's/\r$//' mvnw && chmod +x mvnw \
    && bash ./mvnw dependency:go-offline -pl proverlap-server -s /usr/share/maven/ref/settings.xml -q

# 复制源码并编译
COPY proverlap-server/src proverlap-server/src
RUN bash ./mvnw package -DskipTests -pl proverlap-server -s /usr/share/maven/ref/settings.xml -q
